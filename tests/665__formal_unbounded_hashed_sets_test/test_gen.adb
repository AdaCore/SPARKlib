...
procedure Test_Gen with SPARK_Mode is
   package Inst is new Inst_Gen (Modulus);
   use Inst;
   use Inst.Int_Sets;
   ...
   begin
      ...
      begin
         Assert (Is_Empty (Y), "Copy of empty set");
      end;
      Create_Non_Empty_Set (X);
      declare
         Y : constant Set := Copy (X);
      begin
         Assert (Y = X, "Copy of non-empty set");
      end;
   end Test_Copy;
   ...
   begin
      ...
      Replace_Element (X, P3, 103);

      Assert (Element (X, P3) = 103, "Replace_Element in same bucket, the element is replaced");
      Assert (Length (X) = 6, "Replace_Element in same bucket, length is preserved");
      Assert (Element (X, P1) = 1, "Replace_Element in same bucket, other elements are preserved");

      Replace_Element (X, P3, 0);

      Assert (Element (X, P3) = 0, "Replace_Element in other bucket, the element is replaced");
      Assert (Length (X) = 6, "Replace_Element in other bucket, length is preserved");
      Assert (Element (X, P1) = 1, "Replace_Element in other bucket, other elements are preserved");
   ...
   begin
      ...
      Assert (Element (X, P) = 1003, "Replace, element is updated");
      Assert (Element (X, Q) = 1, "Replace, other elements are preserved");
   end Test_Replace;
   ...
   begin
      ...
      Assert (Length (X) = 5, "Exclude, element not is set");

      Q := Find (X, 1);
      Exclude (X, 1003);

      Assert (Length (X) = 4, "Exclude, length is decremented");
      Assert (not Contains (X, 3), "Exclude, element is removed");
      Assert (Element (X, Q) = 1, "Exclude, other elements are preserved");
   end Test_Exclude;
   ...
   begin
      ...
      Assert (Length (X) = 4, "Delete element, length is decremented");
      Assert (not Contains (X, 3), "Delete, element is removed");
      Assert (Element (X, Q) = 1, "Delete, other elements are preserved");
   end Test_Delete_1;
   ...
   begin
      ...
      Assert (not Contains (X, 3), "Delete cursor, element is removed");
      Assert (Element (X, Q) = 1, "Delete cursor, other elements are preserved");
      ...
      Assert (Length (X) = 4, "Delete last inserted, length is decremented");
      Assert (not Contains (X, 5), "Delete last inserted, element is removed");
   end Test_Delete_2;
   ...
   begin
      ...
      Assert (Find (X, 1) /= No_Element and Find (X, 5) /= No_Element, "Find, element is present");
      Assert (Find (X, 7) = No_Element, "Find, element is not present");
      Assert (Find (X, 1004) /= No_Element, "Find, use provided equivalence function");
   end Test_Find;
   ...
   begin
      ...
      Assert (Contains (X, 1) and Contains (X, 5), "Contains, element is present");
      Assert (not Contains (X, 7), "Contains, element is not present");
      Assert (Contains (X, 1004), "Contains, use provided equivalence function");
   end Test_Contains;
   ...
   begin
      ...
      Assert (Length (X) = 0, "Exclude_Key on empty set");

      Create_Non_Empty_Set (X);
      Int_Sets_Keys.Exclude (X, 6);

      Assert (Length (X) = 5, "Exclude_Key, element not is set");

      Q := Int_Sets_Keys.Find (X, 1);
      Int_Sets_Keys.Exclude (X, 3);

      Assert (Length (X) = 4, "Exclude_Key, length is decremented");
      Assert (not Int_Sets_Keys.Contains (X, 3), "Exclude_Key, element is removed");
      Assert (Element (X, Q) = 1, "Exclude_Key, other elements are preserved");
   ...
   begin
      ...
      Assert (Length (X) = 4, "Delete_Key, length is decremented");
      Assert (not Int_Sets_Keys.Contains (X, 3), "Delete_Key, element is removed");
      Assert (Element (X, Q) = 1, "Delete_Key, other elements are preserved");
   end Test_Delete_Key;
   ...
   begin
      ...
      Assert (Int_Sets_Keys.Find (X, 1) /= No_Element and Int_Sets_Keys.Find (X, 5) /= No_Element, "Find, element is present");
      Assert (Int_Sets_Keys.Find (X, 7) = No_Element, "Find, element is not present");
   end Test_Find_Key;
   ...
   begin
      ...
      Assert (Int_Sets_Keys.Contains (X, 1) and Int_Sets_Keys.Contains (X, 5), "Contains, element is present");
      Assert (not Int_Sets_Keys.Contains (X, 7), "Contains, element is not present");
   ...
   begin
      ...
      Create_Non_Empty_Set (X);
      Assert ((for all P in X => Element (X,  P) /= 0), "for in iteration, non-empty set");
      Assert ((for some E of X => E = 3), "for of iteration, non-empty set");
   end Test_Iteration;
