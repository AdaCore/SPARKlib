...
procedure Test_Gen with SPARK_Mode is
   ...
   procedure Test_Empty_Vector with Pre => True is
      X : Vector := Empty_Vector (1000);
   begin
      Assert (Length (X) = 0, "Empty_Vector is empty");
      Assert (X.Capacity = 1000, "Empty_Vector capacity is capacity parameter");
   ...
   begin
      ...
      Assert (Is_Empty (X), "Reserve_Capacity on empty vector does not do anything");
      Create_Non_Empty_Vector (X);
      Reserve_Capacity (X, 2);
      Reserve_Capacity (X, 200);
      Assert (Length (X) = 5, "Reserve_Capacity on non-empty vector does not do anything");
   ...
   begin
      declare
         Y : constant Vector := Copy (X, 2000);
         Z : constant Vector := Copy (X, 0);
      begin
         Assert (Is_Empty (Y) and Y.Capacity = 2000, "Copy of empty vector");
         Assert (Is_Empty (Z) and Z.Capacity = 0, "Copy of empty vector with default capacity uses Length");
      end;
      Create_Non_Empty_Vector (X);
      declare
         Y : constant Vector := Copy (X, 2000);
         Z : constant Vector := Copy (X, 0);
      begin
         Assert (Y = X and Y.Capacity = 2000, "Copy of non-empty vector");
         Assert (Z = X and Z.Capacity = 5, "Copy of non-empty vector with default capacity uses Length");
      end;
   end Test_Copy;
