...
procedure Test with SPARK_Mode is
   ...
   procedure Test_Empty_List with Pre => True is
      X : List := Empty_List (1000);
   begin
      Assert (Length (X) = 0, "Empty_List is empty");
      Assert (X.Capacity = 1000, "Empty_List capacity is capacity parameter");
   end Test_Empty_List;
   ...
   begin
      ...
      Assert (Length (X) = 100, "Clear does not lose any possible list cell");
   end Test_Clear;
   ...
   begin
      declare
         Y : constant List := Copy (X, 2000);
         Z : constant List := Copy (X, 0);
      begin
         Assert (Is_Empty (Y) and Y.Capacity = 2000, "Copy of empty list");
         Assert (Is_Empty (Z) and Z.Capacity = 1000, "Copy of empty list with preserved capacity");
      end;
      Create_Non_Empty_List (X);
      declare
         Y : constant List := Copy (X, 2000);
         Z : constant List := Copy (X, 0);
      begin
         Assert (Y = X and Y.Capacity = 2000, "Copy of non-empty list");
         Assert (Z = X and Z.Capacity = 1000, "Copy of non-empty list with preserved capacity");
      end;
      Create_Non_Empty_List_With_Holes (W);
      declare
         Y : constant List := Copy (W, 2000);
         Z : constant List := Copy (W, 0);
      begin
         Assert (Y = W and Y.Capacity = 2000, "Copy of non-empty list with holes");
         Assert (Z = W and Z.Capacity = 1000, "Copy of non-empty list with holes with preserved capacity");
      end;
   end Test_Copy;
