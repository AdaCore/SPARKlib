...
procedure Test_Gen with SPARK_Mode is
   ...
   begin
      ...
      Assert (Length (X) = 5, "Exclude, key not is map");
   ...
   begin
      ...
      Assert (not Contains (X, 3), "Delete, key is removed");
      Assert (Key (X, Q) = 1, "Delete, other keys are preserved");
      Assert (Element (X, 1) = 1, "Delete, other mappings are preserved");
   end Test_Delete_1;
   ...
   begin
      ...
      Assert (Find (X, 1) /= No_Element and Find (X, 5) /= No_Element, "Fin, key is present");
