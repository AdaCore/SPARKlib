...
generic
   ...
   with
     procedure Hash_Equivalent (X, Y : Key_Type) is null
     with Ghost => Static;

package SPARK.Containers.Formal.Hashed_Maps with SPARK_Mode, Always_Terminates
is
   ...
   is
      ...
      function K_Keys_Included
        (Left : K.Sequence; Right : K.Sequence) return Boolean
      with
        Ghost  => SPARKlib_Full,
        Global => null,
        Post   =>
   ...
   with
     ...
     Post           =>
       (SPARKlib_Full =>
          ...
          and Equivalent_Keys (Hashed_Maps.Key (Container, Position), Key)),
     Contract_Cases =>
       (SPARKlib_Full =>

          --  If Key is already in Container, it is not modified and Inserted
          --  is set to False.
          ...
          (Contains (Container, Key) =>
           ...
           others                    =>
             ...
             and Key_Logic_Equal
                   (Hashed_Maps.Key (Container, Position),
                    K.Copy_Element (Key))
             and Element_Logic_Equal
   ...
   with
     ...
     Contract_Cases =>
       (SPARKlib_Full =>
          ...
          (not Contains (Model (Container), Key) => Find'Result = No_Element,
