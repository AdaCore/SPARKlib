...
is
      ...
      is
         C : constant Integer :=
           (if (Choose (A1 (I)) and Choose (A2 (I)))
              or (not Choose (A1 (I)) and not Choose (A2 (I)))
            then 0
            elsif Choose (A1 (I))
            then 1
            else -1);
   ...
   package body Count_2 is
      ...
      is
         ...
         is
            ...
            ((Fold_Count.Acc.Fold (A, 0) (I, J)
             ...
             = ((if I > A'First (1)
                ...
                and (for all L in A'First (2) .. J => Choose (A (I, L)))))
         with
           Pre  => I in A'Range (1) and then J in A'Range (2),
           Post =>
             (if J = A'Last (2) then Count_Length'Result = Count_Length (I));

         function Count_Length (I : Index_1) return Boolean
         is
         ...
         procedure Prove_Next (I : Index_1; J : Index_2)
         with
      ...
      begin
         if A'Length (2) > 0 then
            for I in A'Range (1) loop
               pragma
                 Loop_Invariant (if I > A'First (1) then Count_Length (I - 1));
               for J in A'Range (2) loop
                  Prove_Next (I, J);
                  pragma Loop_Invariant (Count_Length (I, J));
               end loop;
            end loop;
         end if;
      end Count_Length;
      ...
      is
         ...
         function Update_Count (K : Index_1; L : Index_2) return Boolean
         is
   ...
   package body Fold_2_Acc is
      ...
      begin
         return
            R : Acc_Array (A'Range (1), A'Range (2)) :=
              (others => (others => Init))
         do
            for I in A'Range (1) loop
               pragma
      ...
      package body Big_Integer_Sum is
         ...
         is
            ...
            is (Fold_Sum.Acc.Fold (A, 0) (I, J)
                = To_Big (C)
                  * ((To_Big_1 (I) - To_Big_1 (A'First (1)))
                     * Length_2 (A)
                     + (To_Big_2 (J) - To_Big_2 (A'First (2))))
                  + To_Big (C))
