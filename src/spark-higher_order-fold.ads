...
package SPARK.Higher_Order.Fold
  with SPARK_Mode
is
   ...
   generic
      ...
      with
        function Final_Prop (A : Array_Type; X : Element_Out) return Boolean
        with Ghost;
   ...
   package Fold_Left_Acc with Always_Terminates
   is
   ...
   package Fold_Left with Always_Terminates
   is
      ...
      with
        ...
        Post =>
          Fold'Result
          = (if A'Length = 0 then Init else Acc.Fold (A, Init) (A'Last));
   end Fold_Left;
   ...
   generic
      ...
      with
        function F
          (X : Element_In; K : Index_Type; I : Element_Out) return Element_Out;
      --  Function that should be applied to elements of Array_Type

   package Fold_Left_I_Acc with Always_Terminates
   is
   ...
   package Fold_Left_I with Always_Terminates
   is
   ...
   package Fold_Right_Acc with Always_Terminates
   is
   ...
   package Fold_Right with Always_Terminates
   is
      ...
      with
        ...
        Post =>
          Fold'Result
          = (if A'Length = 0 then Init else Acc.Fold (A, Init) (A'First));
   end Fold_Right;
   ...
   generic
      ...
      Zero : Element_Out;
      with
        function To_Big (X : Element_Out) return Big_Integer
        with Ghost;
      with
   ...
   package Sum with Always_Terminates is
      ...
      function "+" (Left, Right : Element_Out) return Element_Out
      is (Add (Left, Right))
      with Pre => In_Range (To_Big (Left) + To_Big (Right));
      ...
      procedure Prove_Zero
      with Ghost, Post => To_Big (Zero) = 0;
      --  Axiom: Zero is 0

      package Big_Integer_Sum
        with Ghost
      is
         package Index_Conv is new Signed_Conversions (Index_Type);
         function To_Big_I (X : Index_Type) return Big_Integer
         renames Index_Conv.To_Big_Integer;
         ...
         function Add_Value
           (X : Element_In; Y : Big_Integer) return Big_Integer
         is (To_Big (Value (X)) + Y);
         pragma Annotate (GNATprove, Inline_For_Proof, Entity => Add_Value);

         package Sum_Left is new
           Fold_Left
             (Index_Type  => Index_Type,
              Element_In  => Element_In,
              Array_Type  => Array_Type,
         ...
         function Sum (A : Array_Type) return Big_Integer
         is (Sum_Left.Fold (A, 0));
         ...
         function Length (A : Array_Type) return Big_Integer
         is (if A'Last < A'First
             then 0
         ...
         with
           Post =>
             (if (for all I in A'Range => Value (A (I)) = C)
              then Sum (A) = To_Big (C) * Length (A));
      ...
      function No_Overflows
        (A : Array_Type; X : Element_Out; I : Index_Type) return Boolean
      with Ghost, Subprogram_Variant => (Increases => I), Pre => I in A'Range;
   ...
   package Count with Always_Terminates
   is
      ...
      function In_Range_Last (A : Array_Type; X : Natural) return Boolean
      is (X <= A'Length)
      with Ghost;
      ...
      function Count (A : Array_Type) return Natural
      is (Count_Left.Fold (A, 0));
      ...
      with
        ...
        Pre            =>
          ...
          and then A1'First = A2'First
          and then A1'Last = A2'Last
          and then (for all K in A1'Range => (if K /= I then A1 (K) = A2 (K))),
        Contract_Cases =>
          (Choose (A1 (I)) = Choose (A2 (I))       => Count (A1) = Count (A2),
      ...
      with
        Ghost,
        Post => (Count (A) = 0) = (for all I in A'Range => not Choose (A (I)));
      ...
      with
        Ghost,
        Post =>
          (Count (A) = A'Length) = (for all I in A'Range => Choose (A (I)));
   ...
   generic
      ...
      type Element_In is private;
      type Array_Type is
        array (Index_1 range <>, Index_2 range <>) of Element_In;
      type Element_Out is private;
      with
        function Ind_Prop
   ...
   package Fold_2_Acc with Always_Terminates
   is
      ...
      type Acc_Array is
        array (Index_1 range <>, Index_2 range <>) of Element_Out;
   ...
   package Fold_2 with Always_Terminates
   is
      ...
      with
        ...
        Post =>
          (if A'Length (1) = 0 or else A'Length (2) = 0
           then Fold'Result = Init
           else
             Fold'Result = Acc.Fold (A, Init) (A'Last (1), A'Last (2))
   ...
   package Sum_2 with Always_Terminates is
      ...
      is
         ...
         function To_Big_1 (X : Index_1) return Big_Integer
         renames Index_Conv_1.To_Big_Integer;
         package Index_Conv_2 is new Signed_Conversions (Index_2);
         function To_Big_2 (X : Index_2) return Big_Integer
         renames Index_Conv_2.To_Big_Integer;
         ...
         function Length_1 (A : Array_Type) return Big_Integer
         is (if A'Last (1) < A'First (1)
             then 0
             else To_Big_1 (A'Last (1)) - To_Big_1 (A'First (1)) + 1);

         function Length_2 (A : Array_Type) return Big_Integer
         is (if A'Last (2) < A'First (2)
             then 0
   ...
   generic
      ...
      type Index_2 is range <>;
      type Element is private;
      type Array_Type is array (Index_1 range <>, Index_2 range <>) of Element;
      with function Choose (X : Element) return Boolean;
      --  Count the number of elements for which Choose return True

   package Count_2 with Always_Terminates
   is
      ...
      function Result_In_Range (A : Array_Type; X : Natural) return Boolean
      is (X <= A'Length (1) * A'Length (2));

      package Fold_Count is new
        Fold_2
          (Index_1     => Index_1,
           Index_2     => Index_2,
           Element_In  => Element,
