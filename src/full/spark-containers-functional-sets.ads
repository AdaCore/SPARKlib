...
package SPARK.Containers.Functional.Sets with SPARK_Mode, Always_Terminates
is
   ...
   package Renamings is
      ...
      function "=" (Left : Element_Type; Right : Element_Type) return Boolean
   ...
   with
     ...
     Aggregate                 =>
       (Empty => Empty_Set, Add_Unnamed => Aggr_Include),
     Annotate                  =>
   ...
   with
     ...
     Pre      =>
       Enable_Handling_Of_Equivalence and then not Contains (Container, Item),
   ...
   function Set_Logic_Equal (Left, Right : Set) return Boolean
   with Ghost => SPARKlib_Full, Annotate => (GNATprove, Logical_Equal);
   --  Logical equality on sets

   function Iterate (Container : Set) return Iterable_Set
   with
     Global => null,
     Post   =>
       (SPARKlib_Full =>
   ...
   function Get_Set (Iterator : Iterable_Set) return Set
   with Global => null;
   ...
   function First (Iterator : Iterable_Set) return Set
   with
     Global => null,
     Post   =>
       (SPARKlib_Full =>
          Set_Logic_Equal (First'Result, Get_Set (Iterator))
          and then Valid_Subset (Iterator, First'Result));
   --  In the first iteration, the cursor is the set associated with Iterator

   function Next (Iterator : Iterable_Set; Cursor : Set) return Set
   with
     Global => null,
     Pre    =>
       (SPARKlib_Defensive =>
   ...
   function Has_Element (Iterator : Iterable_Set; Cursor : Set) return Boolean
   with
     Global => null,
     Post   =>
       (SPARKlib_Full =>
   ...
   function Iter_First (Container : Set) return Private_Key
   with Ghost => SPARKlib_Full, Global => null;

   function Iter_Has_Element
     (Container : Set; Key : Private_Key) return Boolean
   with Ghost => SPARKlib_Full, Global => null;
   ...
   function Iter_Element
     (Container : Set; Key : Private_Key) return Element_Type
   with
     Ghost  => SPARKlib_Full,
     Global => null,
     Pre    => Iter_Has_Element (Container, Key);
   ...
   procedure Aggr_Include (Container : in out Set; Item : Element_Type)
   with
     Global => null,
     Pre    => (SPARKlib_Defensive => not Contains (Container, Item)),
     Post   =>
...
private
   ...
   function Has_Element (Iterator : Iterable_Set; Cursor : Set) return Boolean
   is (Valid_Subset (Iterator, Cursor) and then Length (Cursor) > 0);

   function Iterate (Container : Set) return Iterable_Set
   is (Iterable_Set'(Container => Container));

   function Next (Iterator : Iterable_Set; Cursor : Set) return Set
   is (Remove (Cursor, Choose (Cursor)));

   function Set_Logic_Equal (Left, Right : Set) return Boolean
   is (Ptr_Eq (Left.Content, Right.Content)
