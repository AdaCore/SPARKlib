...
package SPARK.Containers.Formal.Vectors with SPARK_Mode, Always_Terminates
is
   ...
   with
     Global => null,
     Post   => (SPARKlib_Full => First_Index'Result = Index_Type'First);
   pragma Annotate (GNATprove, Inline_For_Proof, Entity => First_Index);
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          Index_Type'Pos (Last_Index'Result)
   ...
   is
      ...
      function Of_Big (J : Big_Integer) return Count_Type
      renames Conversions.From_Big_Integer;
      ...
      function "=" (Left : M.Sequence; Right : M.Sequence) return Boolean
      renames M."=";
      ...
      pragma
        Annotate (GNATprove, Inline_For_Proof, Entity => M_Elements_Included);
      ...
      pragma
        Annotate (GNATprove, Inline_For_Proof, Entity => M_Elements_Swapped);
      ...
      with
        ...
        Post   =>
          (SPARKlib_Full =>
             M.Last (Model'Result) = Last_Index (Container)
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          Length'Result = Of_Big (M.Length (Model (Container))));
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          "="'Result = (M.Equivalent_Sequences (Model (Left), Model (Right))));
   ...
   with
     Global => null,
     Post   => (SPARKlib_Full => Capacity'Result = Container.Capacity);
   pragma Annotate (GNATprove, Inline_For_Proof, Entity => Capacity);

   procedure Reserve_Capacity
     (Container : in out Vector; Capacity : Capacity_Range)
   with
     Global => null,
     Pre    => (SPARKlib_Defensive => Capacity <= Container.Capacity),
     Post   =>
   ...
   with Global => null, Post => (SPARKlib_Full => Length (Container) = 0);

   procedure Assign (Target : in out Vector; Source : Vector)
   with
     Global => null,
     Pre    => (SPARKlib_Defensive => Length (Source) <= Target.Capacity),
   ...
   with
     Global => null,
     Pre    =>
       (SPARKlib_Defensive => (Capacity = 0 or Length (Source) <= Capacity)),
     Post   =>
       (SPARKlib_Full =>
   ...
   with
     Global => null,
     Pre    => (SPARKlib_Defensive => Length (Source) <= Capacity (Target)),
     Post   =>
       (SPARKlib_Full =>
   ...
   with
     ...
     Post     =>
       (SPARKlib_Full => Element'Result = Element (Model (Container), Index)),
   ...
   with
     Global => null,
     Pre    =>
       (SPARKlib_Defensive => Length (Container) < Capacity (Container)),
     Post   =>
       (SPARKlib_Full =>
          Length (Container) = Length (Container)'Old + 1
   ...
   with
     ...
     Post           =>
       (SPARKlib_Full =>
          Length (Container)
          in Length (Container)'Old - Count .. Length (Container)'Old
   ...
   with
     Global => null,
     Pre    => (SPARKlib_Defensive => Length (Container) > 0),
     Post   =>
       (SPARKlib_Full =>
          Length (Container) = Length (Container)'Old - 1
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          ...
          = Element (Model (Container), Index_Type'First));
   pragma Annotate (GNATprove, Inline_For_Proof, Entity => First_Element);
   ...
   pragma Annotate (GNATprove, Inline_For_Proof, Entity => Last_Element);
       ...
       (SPARKlib_Full =>
          ...
          (not M.Contains
                 (Container => Model (Container),
                  ...
                  Item      => Item) => Reverse_Find_Index'Result = No_Index,
   ...
   package Generic_Sorting with SPARK_Mode, Always_Terminates is
      ...
      is
         ...
         pragma
           Annotate (GNATprove, Inline_For_Proof, Entity => M_Elements_Sorted);
      ...
      with
        ...
        Pre    =>
          (SPARKlib_Defensive =>
             Length (Source) <= Capacity (Target) - Length (Target)),
        Post   =>
          (SPARKlib_Full =>
   ...
   function Aggr_Capacity (Container : Vector) return Count_Type
   ...
   function Aggr_And_Iter_Model (Container : Vector) return M.Sequence
   is (Model (Container))
   with
     Ghost    => Static,
     Global   => null,
