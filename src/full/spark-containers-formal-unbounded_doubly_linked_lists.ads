...
is
   ...
   with
     Global => null,
     Post   => (SPARKlib_Full => Length (Empty_List'Result) = 0);
   ...
   is
      ...
      procedure Lift_Abstraction_Level (Container : List)
      with
       ...
       (SPARKlib_Full =>
          ...
          and
            (if Before = No_Element
             then P.Get (Positions (Container), Position) = Length (Container)
             else
               P.Get (Positions (Container), Position)
               = P.Get (Positions (Container)'Old, Before))
   ...
   with
     Global => null,
     Pre    => (SPARKlib_Defensive => Length (Container) < Count_Type'Last),
     Post   =>
       (SPARKlib_Full =>
          Length (Container) = Length (Container)'Old + 1
          ...
          and
            P_Positions_Shifted
              (Positions (Container)'Old, Positions (Container), Cut => 1));
   ...
   with
     Global => null,
     Pre    =>
       (SPARKlib_Defensive => Length (Container) <= Count_Type'Last - Count),
     Post   =>
       (SPARKlib_Full =>
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          ...
          and
            P_Positions_Shifted
              (Positions (Container), Positions (Container)'Old, Cut => 1));
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          ...
          and
            (if Before = No_Element
             then P.Get (Positions (Target), Position) = Length (Target)
             else
          ...
          and
            Element_Logic_Equal
              (Element (Model (Target), P.Get (Positions (Target), Position)),
               Element
                 (Model (Source)'Old,
       ...
       (SPARKlib_Full =>
          ...
          (not M.Contains
                 (Container => Model (Container),
                  ...
                  Lst       => Length (Container),
                  Item      => Item) => Find'Result = No_Element,
       ...
       (SPARKlib_Full =>
          ...
          (not M.Contains
                 (Container => Model (Container),
                  ...
                  Item      => Item) => Reverse_Find'Result = No_Element,
           ...
           others                    =>
             ...
             --  The element designated by the result of Reverse_Find is
             --  Item.
             ...
             and
               not M.Contains
                     (Container => Model (Container),
                      Fst       =>
                        P.Get (Positions (Container), Reverse_Find'Result) + 1,
                      Lst       =>
                        (if Position = No_Element
                         then Length (Container)
                         else P.Get (Positions (Container), Position)),
   ...
   package Generic_Sorting with SPARK_Mode, Always_Terminates
   is
   ...
   function Aggr_And_Iter_Model (Container : List) return M.Sequence
   is (Model (Container))
   with
     Ghost    => Static,
     Global   => null,
