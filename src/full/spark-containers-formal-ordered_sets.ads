...
generic
   ...
   with
     procedure Lt_Irreflexive (X, Y : Element_Type) is null
     with Ghost => Static;
   with
     procedure Lt_Asymmetric (X, Y : Element_Type) is null
     with Ghost => Static;
   with
     procedure Lt_Transitive (X, Y, Z : Element_Type) is null
     with Ghost => Static;
   with
     procedure Lt_Order (X, Y, Z : Element_Type) is null
     with Ghost => Static;

package SPARK.Containers.Formal.Ordered_Sets with SPARK_Mode, Always_Terminates
is
   ...
   pragma
     Annotate (GNATprove, Inline_For_Proof, Entity => Equivalent_Elements);
   ...
   is
      ...
      with
        Global => null,
        Pre    => (SPARKlib_Defensive => Lst <= E.Last (Container)),
        Post   =>
          (SPARKlib_Full =>
             E_Bigger_Than_Range'Result
             = (for all I in Fst .. Lst => E.Get (Container, I) < Item));
      pragma
        Annotate (GNATprove, Inline_For_Proof, Entity => E_Bigger_Than_Range);
      ...
      pragma
        Annotate (GNATprove, Inline_For_Proof, Entity => E_Smaller_Than_Range);
      ...
      with
        Global => null,
        Pre    => (SPARKlib_Defensive => Position - 1 <= E.Last (Container)),
        Post   =>
          (SPARKlib_Full =>
             E_Is_Find'Result
             =
             ...
             ((if Position > 0
               then E_Bigger_Than_Range (Container, 1, Position - 1, Item))

              and
                (if Position < E.Last (Container)
                 then
                   E_Smaller_Than_Range
                     (Container, Position + 1, E.Last (Container), Item))));
      pragma Annotate (GNATprove, Inline_For_Proof, Entity => E_Is_Find);
      ...
      with
        ...
        Post   =>
          (SPARKlib_Full =>
             M.Length (Model'Result) = E.Big (Length (Container)));
     ...
     Post   =>
       (SPARKlib_Full =>
          ...
          (if "="'Result
           ...
           --  If they are different, then they do not contain the same
           --  elements.
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          Equivalent_Sets'Result = (Model (Left) = Model (Right)));
     ...
     Post   =>
       (SPARKlib_Full =>
          ...
          and
            Element_Logic_Equal
              (E.Get
                 (Elements (Container), Find (Elements (Container), New_Item)),
               E.Copy_Element (New_Item))
   ...
   with
     ...
     Pre            =>
       (SPARKlib_Defensive =>
          Length (Container) < Container.Capacity
          or Contains (Container, New_Item)),
     Post           => (SPARKlib_Full => Contains (Container, New_Item)),
     Contract_Cases =>
       (SPARKlib_Full =>
          ...
          (Contains (Container, New_Item) =>
             ...
             and
               ...
               = Positions (Container)'Old
                 ...
                 --  The element equivalent to New_Item in Container is
           ...
           others                         =>
             ...
             --  The Elements of Container located before New_Item are
             --  preserved.
   ...
   with
     Global         => null,
     Post           => (SPARKlib_Full => not Contains (Container, Item)),
     Contract_Cases =>
       (SPARKlib_Full =>
   ...
   with
     ...
     Pre    =>
       (SPARKlib_Defensive =>
          Length (Left) <= Count_Type'Last - Length (Right)),
     Post   =>
       (SPARKlib_Full =>
          E.Big (Length (Union'Result))
          = E.Big (Length (Left))
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          E.Big (Length (Difference'Result))
          = E.Big (Length (Left))
            - M.Num_Overlaps (Model (Left), Model (Right))
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          E.Big (Length (Symmetric_Difference'Result))
          = E.Big (Length (Left))
   ...
   with
     ...
     Post   =>
       (SPARKlib_Full =>
          Is_Subset'Result = (Model (Subset) <= Model (Of_Set)));
   ...
   with
     Global => null,
     Post   =>
       (SPARKlib_Full => Contains'Result = Contains (Model (Container), Item));
   ...
   generic
      ...
      with
        procedure Lt_Compatible (X, Y : Element_Type) is null
        with Ghost => Static;

   package Generic_Keys with SPARK_Mode, Always_Terminates
   is
      ...
      is
         ...
         pragma
           Annotate
             (GNATprove, Inline_For_Proof, Entity => E_Bigger_Than_Range);
         ...
         pragma
           Annotate
             (GNATprove, Inline_For_Proof, Entity => E_Smaller_Than_Range);
         ...
         with
           Global => null,
           Pre    =>
             (SPARKlib_Defensive => Position - 1 <= E.Last (Container)),
           Post   =>
             (SPARKlib_Full =>
                E_Is_Find'Result
                =
                ...
                ((if Position > 0
                  then E_Bigger_Than_Range (Container, 1, Position - 1, Key))

                 and
                   (if Position < E.Last (Container)
                    then
                      E_Smaller_Than_Range
                        (Container, Position + 1, E.Last (Container), Key))));
         pragma Annotate (GNATprove, Inline_For_Proof, Entity => E_Is_Find);

         function Find
           (Container : E.Sequence; Key : Key_Type) return Count_Type
         with
           Global => null,
           Post   =>
             (SPARKlib_Full =>
         ...
         function M_Included_Except
           (Left : M.Set; Right : M.Set; Key : Key_Type) return Boolean
         with
           Global => null,
           Post   =>
             (SPARKlib_Full =>
      ...
      with
        ...
        Pre    => (SPARKlib_Defensive => Has_Element (Container, Position)),
        Post   =>
          (SPARKlib_Full => Key'Result = Key (Element (Container, Position)));
      ...
      with
        Global         => null,
        Post           => (SPARKlib_Full => not Contains (Container, Key)),
        Contract_Cases =>
          (SPARKlib_Full =>
             ...
             (not Contains (Container, Key) =>
              ...
              others                        =>
                ...
                --  The elements of Container located before Key are
                --  preserved.
          ...
          (SPARKlib_Full =>
             ...
             ((for all E of Model (Container) =>
              ...
              others                                              =>
                ...
                and
                  Equivalent_Keys
                    (Generic_Keys.Key (Element (Container, Find'Result)),
                     Key)));
   ...
   function Aggr_Capacity (Container : Set) return Count_Type
...
private
   ...
   package Tree_Types is new
     Red_Black_Trees.Generic_Bounded_Tree_Types (Node_Type);
