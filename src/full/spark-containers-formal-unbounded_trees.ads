...
package SPARK.Containers.Formal.Unbounded_Trees with
    SPARK_Mode,
    Always_Terminates
is
   pragma Annotate (CodePeer, Skip_Analysis);

   type Tree is private with Default_Initial_Condition => Is_Empty (Tree);
   ...
   with
     Global => null,
     Post   => (SPARKlib_Full => Is_Empty (Empty_Tree'Result));
   ...
   with
     Global => null,
     Pre    => (SPARKlib_Defensive => Is_Empty (Container)),
     Post   =>
       (SPARKlib_Full =>
   ...
   with
     ...
     Pre    =>
       (SPARKlib_Defensive =>
          Has_Element (Container, Parent)
          and then Node_Count (Container) < Count_Type'Last
...
private
   ...
   package Element_Holder_Types is new
     SPARK.Containers.Formal.Holders (Element_Type);
   ...
   type Cell_Type is record
      ...
      --  Holds the element for this node.
   end record;
   ...
   type Tree is new Ada.Finalization.Controlled with record
      Children   : Child_Maps.Map := Child_Maps.Empty_Map;
   ...
   overriding
   procedure Adjust (Object : in out Tree);
