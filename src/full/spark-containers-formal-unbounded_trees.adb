--
--  Copyright (C) 2027, Free Software Foundation, Inc.
--
...
package body SPARK.Containers.Formal.Unbounded_Trees
  with SPARK_Mode => Off
is
   ...
   procedure Deallocate_Cells is new
     Ada.Unchecked_Deallocation
       (Object => Cell_Array,
        Name   => Cell_Array_Access);
      ...
      function Model (T : Tree) return M.Tree is
         ...
         begin
            if C = No_Element then
               ...
               Result :=
                 M.Create
                   (EHT.Element_Access (T.Cells.all (C.Node).E_Holder).all,
                    Children);
            end if;
      ...
      begin
         if Is_Empty (T) then
            ...
            while Node /= No_Element loop
               ...
               ST :=
                 M.Create
                   (EHT.Element_Access (T.Cells.all (Node.Node).E_Holder).all,
                    Children);
   ...
   begin
      ...
      Tree_Traversal_Loop : loop
         ...
         if EHT.Element_Access (Left.Cells.all (L_Node.Node).E_Holder).all
           /= EHT.Element_Access (Right.Cells.all (R_Node.Node).E_Holder).all
         then
            return False;
         end if;
   ...
   overriding
   procedure Adjust (Object : in out Tree) is
      Cells_Copy : Cell_Array_Access;
   begin
      if Object.Cells /= null then
         Cells_Copy := new Cell_Array (Object.Cells.all'Range);
         Cells_Copy.all := Object.Cells.all;
         Object.Cells := Cells_Copy;
      end if;
   end Adjust;
   ...
   procedure Allocate_Cell
     (Container : in out Tree; Index : out Positive_Count_Type)
   is
      Current_Capacity : Positive_Count_Type;
      New_Capacity     : Positive_Count_Type;
      Cells_Copy       : Cell_Array_Access;
      Old_Cells        : Cell_Array_Access;

   begin
      if Container.Cells = null then

         --  This is the case when the tree is empty

         Container.Cells := new Cell_Array (1 .. 1);
         Index := 1;
         Container.Used_Cells := 1;

      elsif Container.Free_List /= No_Element then

         --  This is the case when the free list is non-empty.
         --
         --  Prefer allocating from the free list first to avoid extending
         --  the cells array until absolutely necessary.

         Index := Container.Free_List.Node;
         Container.Free_List := Container.Cells.all (Index).Parent;

      elsif Container.Used_Cells < Container.Cells.all'Length then
         ...
         --  This is the case when the cells array is completely full, so the
         --  array needs to be resized.
         --
         --  In order to guarantee that the amortized allocation cost is O(1),
         --  we always allocate an array whose length is some power-of-two
         --  factor of the current array length.

         Current_Capacity := Container.Cells.all'Last;

         if Current_Capacity <= Count_Type'Last / 2 then
            New_Capacity := Current_Capacity * 2;
         else
            New_Capacity := Positive_Count_Type'Last;
         end if;

         Cells_Copy := new Cell_Array (1 .. New_Capacity);
         Cells_Copy.all (1 .. Current_Capacity) := Container.Cells.all;
         Old_Cells := Container.Cells;
         Container.Cells := Cells_Copy;

         Deallocate_Cells (Old_Cells);
   ...
   begin
      ...
      if not Is_Empty (Source) then
         ...
         --  Target.Cells should be set to null by Clear

         pragma Assert (Target.Cells = null);

         Target.Cells := new Cell_Array'(Source.Cells.all);
         Target.Root := Source.Root;
         Target.Free_List := Source.Free_List;
         Target.Node_Count := Source.Node_Count;
         Target.Used_Cells := Source.Used_Cells;

         Child_Maps.Assign
           (Target => Target.Children, Source => Source.Children);
      end if;
   end Assign;
   ...
   begin
      if Container.Cells /= null then
         Deallocate_Cells (Container.Cells);
      end if;
   ...
   begin
      ...
      return EHT.Element_Access (Container.Cells.all (Position.Node).E_Holder);
   ...
   begin
      return Result : Tree do
         Assign (Target => Result, Source => Source);
      end return;
   end Copy;
   ...
   procedure Deallocate_Leaf (Container : in out Tree; Position : Cursor) is
      Cell_Ref : Cell_Type renames Container.Cells.all (Position.Node);
   begin
      EHT.Finalize (Cell_Ref.E_Holder);
   ...
   begin
      ...
      loop
         Next := Container.Cells.all (Next.Node).Parent;
         exit when Next = No_Element;
         Result := Result + 1;
      end loop;
   ...
   function Direction_Nocheck
     (Container : Tree; Position : Cursor) return Way_Type
   is (Container.Cells.all (Position.Node).Direction);
   ...
   begin
      ...
      return
        EHT.Element_Access (Container.Cells.all (Position.Node).E_Holder).all;
   end Element;
   ...
   function First_Child_Nocheck
     (Container : Tree; Position : Cursor) return Cursor
   is (Container.Cells.all (Position.Node).First_Child);
   ...
   is (Container.Cells /= null
       and then Position.Node in Container.Cells.all'Range
       and then not Container.Cells.all (Position.Node).Free);
   ...
   begin
      ...
      if Container.Node_Count = Count_Type'Last then
      ...
      EHT.Replace_Element
        (Container.Cells.all (New_Cell_Index).E_Holder, New_Item);
   ...
   begin
      ...
      EHT.Replace_Element
        (Container.Cells.all (New_Parent.Node).E_Holder, New_Item);
   ...
   begin
      ...
      return Container.Cells.all (Position.Node).First_Child = No_Element;
   end Is_Leaf;
   ...
   begin
      ...
      return Container.Cells.all (Position.Node).Last_Child;
   end Last_Child;
   ...
   is
      Parent_Ref : Cell_Type renames Container.Cells.all (Parent.Node);
      Child_Ref  : Cell_Type renames Container.Cells.all (Child.Node);
      Inserted   : Boolean;
   begin
   ...
   begin
      ...
      if Target.Cells /= null then
         Deallocate_Cells (Target.Cells);
      end if;
      ...
      Source.Cells := null;
      Source.Root := No_Element;
      Source.Free_List := No_Element;
      Source.Node_Count := 0;
      Source.Used_Cells := 0;

      Child_Maps.Move (Target => Target.Children, Source => Source.Children);
   end Move;
   ...
   function Next_Sibling_Nocheck
     (Container : Tree; Position : Cursor) return Cursor
   is
      Node_Ref : Cell_Type renames Container.Cells.all (Position.Node);
   ...
   begin
      ...
      if Position = Container.Cells.all (Node_Ref.Parent.Node).Last_Child then
         return No_Element;
      end if;
   ...
   function Parent_Nocheck (Container : Tree; Position : Cursor) return Cursor
   is (Container.Cells.all (Position.Node).Parent);
   ...
   begin
      ...
      if Position = Container.Cells.all (Node_Ref.Parent.Node).First_Child then
         return No_Element;
      end if;
   ...
   function Reference
     (Container : Tree; Position : Cursor) return not null access Element_Type
   is
   begin
      if not Has_Element (Container, Position) then
         raise Constraint_Error with "Position cursor not in tree";
   ...
   begin
      ...
      EHT.Replace_Element
        (Container.Cells.all (Position.Node).E_Holder, New_Item);
   end Replace_Element;
      ...
      if Cell_Ref.Parent /= No_Element then
         declare
            Parent_Ref : Cell_Type renames
              Container.Cells.all (Cell_Ref.Parent.Node);
         begin
   ...
   procedure Unlink_And_Replace
     (Container : in out Tree; Old_Position : Cursor; New_Position : Cursor)
   is
      Old_Ref : Cell_Type renames Container.Cells.all (Old_Position.Node);
      New_Ref : Cell_Type renames Container.Cells.all (New_Position.Node);
   begin
      ...
      if Old_Ref.Parent /= No_Element then
         ...
         declare
            Parent_Ref : Cell_Type renames
              Container.Cells.all (Old_Ref.Parent.Node);
         begin
...
end SPARK.Containers.Formal.Unbounded_Trees;
