...
package SPARK.Containers.Functional.Maps with SPARK_Mode, Always_Terminates
is
   ...
   with
     ...
     Aggregate                 =>
       (Empty => Empty_Map, Add_Named => Aggr_Include),
     Annotate                  =>
   ...
   with
     ...
     Pre      =>
       Enable_Handling_Of_Equivalence and then not Has_Key (Container, Key),
   ...
   function Length (Container : Map) return Big_Natural
   with
     Global   => null,
   ...
   function Map_Logic_Equal (Left, Right : Map) return Boolean
   with Ghost => SPARKlib_Full, Annotate => (GNATprove, Logical_Equal);
   --  Logical equality on maps

   function Iterate (Container : Map) return Iterable_Map
   with
     Global => null,
     Post   =>
       (SPARKlib_Full =>
   ...
   function Get_Map (Iterator : Iterable_Map) return Map
   with Global => null;
   --  Retrieve the map associated with an iterator

   function Valid_Submap (Iterator : Iterable_Map; Cursor : Map) return Boolean
   with
     Global => null,
     Post   =>
       (SPARKlib_Full =>
   ...
   function First (Iterator : Iterable_Map) return Map
   with
     Global => null,
     Post   =>
       (SPARKlib_Full =>
   ...
   function Next (Iterator : Iterable_Map; Cursor : Map) return Map
   with
     Global => null,
     Pre    =>
       (SPARKlib_Defensive =>
   ...
   function Has_Element (Iterator : Iterable_Map; Cursor : Map) return Boolean
   with
     Global => null,
     Post   =>
       (SPARKlib_Full =>
   ...
   with
     ...
     Post   =>
       ...
       = (for all Key of Left =>
            Has_Key (Right, Key)
            and then Element_Logic_Equal (Get (Left, Key), Get (Right, Key)));
   ...
   function Iter_First (Container : Map) return Private_Key
   with Ghost => SPARKlib_Full, Global => null;

   function Iter_Has_Element
     (Container : Map; Key : Private_Key) return Boolean
   with Ghost => SPARKlib_Full, Global => null;
...
private
   ...
   function First (Iterator : Iterable_Map) return Map
   is (Iterator.Container);

   function Get_Map (Iterator : Iterable_Map) return Map
   is (Iterator.Container);

   function Has_Element (Iterator : Iterable_Map; Cursor : Map) return Boolean
   is (Valid_Submap (Iterator, Cursor) and then Length (Cursor) > 0);

   function Iterate (Container : Map) return Iterable_Map
   is (Iterable_Map'(Container => Container));

   function Map_Logic_Equal (Left, Right : Map) return Boolean
   is (Ptr_Eq (Left.Keys, Right.Keys)
       and then Length (Left.Keys) = Length (Right.Keys)
       and then Ptr_Eq (Left.Elements, Right.Elements));

   function Next (Iterator : Iterable_Map; Cursor : Map) return Map
   is (Remove (Cursor, Choose (Cursor)));

   function Valid_Submap (Iterator : Iterable_Map; Cursor : Map) return Boolean
   is (Ptr_Eq (Cursor.Keys, Iterator.Container.Keys)
