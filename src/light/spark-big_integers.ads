...
package SPARK.Big_Integers
  with SPARK_Mode, Ghost => SPARKlib_Logic, Pure, Always_Terminates
is

   type Big_Integer is private with Integer_Literal => From_Universal_Image;
   ...
   with
     Dynamic_Predicate => Is_Valid (Valid_Big_Integer),
   ...
   with
     Dynamic_Predicate =>
       (if Is_Valid (Big_Positive) then Big_Positive > To_Big_Integer (0)),
     Predicate_Failure => raise Constraint_Error;

   subtype Big_Natural is Big_Integer
   with
     Dynamic_Predicate =>
       (if Is_Valid (Big_Natural) then Big_Natural >= To_Big_Integer (0)),
     Predicate_Failure => raise Constraint_Error;
   ...
   with
     Pre    =>
       (L /= To_Big_Integer (0) and R /= To_Big_Integer (0))
       or else raise Constraint_Error,
     Global => null;
