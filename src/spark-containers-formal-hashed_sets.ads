...
generic
   ...
   with
     procedure Hash_Equivalent (X, Y : Element_Type) is null
     with Ghost => Static;

package SPARK.Containers.Formal.Hashed_Sets with SPARK_Mode, Always_Terminates
is
   ...
   function Default_Modulus (Capacity : Count_Type) return Pos_Hash_Type
   with Global => null;
   ...
   generic
      ...
      with
        procedure Eq_Compatible (X, Y : Element_Type) is null
        with Ghost => Static;
      ...
      with
        procedure Hash_Compatible (X : Element_Type) is null
        with Ghost => Static;
