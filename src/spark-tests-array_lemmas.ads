...
is
   ...
   procedure Test_Transitive_Order_Int (Arr : Arr_Int_Constrained)
   with
     Global => null,
     Pre    =>
     ...
     Post   =>
       (for all I in Arr'Range =>
          (for all J in Arr'Range => (if I < J then Arr (I) < Arr (J))));

   procedure Test_Transitive_Order_Float (Arr : Arr_Float_Constrained)
   with
     Global => null,
     Pre    =>
       (for all I in Index_Type'First + 1 .. Index_Type'Last =>
   ...
   procedure Test_Transitive_Order_Int (Arr : Arr_Int_Unconstrained)
   with
     Global => null,
     Pre    =>
   ...
   procedure Test_Transitive_Order_Float (Arr : Arr_Float_Unconstrained)
   with
     Global => null,
     Pre    =>
       (for all I in Arr'First .. Arr'Last =>
